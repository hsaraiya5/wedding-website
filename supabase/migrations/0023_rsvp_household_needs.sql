-- v2 handoff's RSVP "Household Needs" block: dietary restrictions/
-- allergies, accessibility/mobility needs, and a general note, collected
-- alongside the existing song request and WhatsApp numbers.

alter table households
  add column dietary_needs text,
  add column accessibility_needs text,
  add column household_note text;
...
drop function if exists public.submit_rsvp(jsonb, text, jsonb);

grant execute on function public.submit_rsvp(jsonb, text, jsonb, text, text, text) to authenticated, anon;
