-- Hard cap: a household can't register more WhatsApp numbers than it has
-- guests on the invitation. Enforced in the RPC (not just the UI) since
-- this is the actual write path guests use.

create or replace function public.submit_rsvp(
  p_answers jsonb,
  p_song_request text default null,
  p_whatsapp_numbers jsonb default null,
  p_dietary_needs text default null,
  p_accessibility_needs text default null,
  p_household_note text default null
)
returns void
language plpgsql
security definer
...
begin
  ...
  if p_whatsapp_numbers is not null then
    select count(*) into v_whatsapp_count
      from jsonb_array_elements(p_whatsapp_numbers) as n
      where coalesce(trim(n ->> 'phone_number'), '') <> '';

    select count(*) into v_guest_count from guests where household_id = v_household_id;

    if v_whatsapp_count > v_guest_count then
      raise exception 'too_many_whatsapp_numbers';
    end if;
  end if;
  ...
  update households
    set rsvp_submitted_at = now(),
        ...
        dietary_needs = nullif(trim(coalesce(p_dietary_needs, '')), ''),
        accessibility_needs = nullif(trim(coalesce(p_accessibility_needs, '')), ''),
        household_note = nullif(trim(coalesce(p_household_note, '')), '')
