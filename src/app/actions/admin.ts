...
export async function saveAboutUs(_prevState: ActionState, formData: FormData): Promise<ActionState> {
  ...
  // Photo files are uploaded to Supabase Storage directly from the browser
  // (see about-us-form.tsx) rather than through this action -- Vercel caps
  // serverless function request bodies well below what several photos in
  // one submission need, independent of Next's own bodySizeLimit config.
  // This action only ever sees the resulting URLs, which stay tiny.
  const photos: { url: string; caption: string | null }[] = [];
  for (const key of photoKeys) {
    const url = String(formData.get(`photo_${key}_existing_url`) ?? "").trim();
